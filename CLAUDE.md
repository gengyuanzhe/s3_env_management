...
client/                 # React前端 (Vite dev server, port 5173)
  src/
    ...
    components/modals/  # 6个弹窗 (AddEnv, EditEnv, Upload, Download, CreateBucket, EditCommand)
...
- `POST /api/s3/:envId/upload` — 上传对象 (multipart, 支持普通/多段模式)
- `GET /api/s3/:envId/download` — 下载对象 (流式返回)
- `DELETE /api/s3/:envId/buckets/:bucket/objects` — 删除对象

## 数据模型

### Environment
字段: id, name, envId, instanceId, model, cpuArch, form, disk, managementUrl, account, password, sdeName, nodes[{name, internalIp, externalIp, credentials}], s3Config{endpoint, ak, sk}, createdAt
...
- 下载优先使用 `showSaveFilePicker` API，不支持时回退标准下载
- Xshell通过 `ssh://` 协议唤起
